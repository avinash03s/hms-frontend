  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <IconChevronDown size={14} className={`text-gray-400 transition-transform duration-200 ${open ? "rotate-180" : ""}`} />
      </button>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            <div className="relative group" onMouseLeave={() => setSelectedCity(null)}>
              <button className="flex items-center gap-1 px-3 py-2 rounded-md text-sm font-medium text-gray-600 hover:text-[#1a6fa8] hover:bg-blue-50 transition-colors">
                      ...
                      {!hospitalsByCity[selectedCity] ? (
                        ...
                        hospitalsByCity[selectedCity].map((h) => (
                          <Link key={h.id} to={`/hospitals/${h.id}`}
                            ...
                            <p className="text-sm font-semibold text-gray-700 group-hover/item:text-[#1a6fa8]">{h.name}</p>
                            <p className="text-xs text-gray-400 truncate mt-0.5" style={{ maxWidth: 180 }}>{h.address}</p>
                          </Link>
                        ))
                      )}
                    </div>
            ...
            <div className="flex gap-2 mt-3 pt-3 border-t border-gray-100">
              {isPatientLoggedIn ? (
                <div className="flex flex-col gap-1 w-full">
                  <Link to="/patient/profile" onClick={() => setMobileOpen(false)}
                  ...
                  <Link to="/patient/appointments" onClick={() => setMobileOpen(false)}
                    className="flex items-center gap-2 px-3 py-2.5 text-sm text-gray-700 hover:bg-blue-50 rounded-md">
                    <IconCalendarTime size={15} stroke={1.5} /> My Appointments
                  </Link>
                  <Link to="/patient/packages" onClick={() => setMobileOpen(false)}
                    className="flex items-center gap-2 px-3 py-2.5 text-sm text-gray-700 hover:bg-blue-50 rounded-md">
                    <IconPackage size={15} stroke={1.5} /> My Packages
                  </Link>
                  <Link to="/patient/reports" onClick={() => setMobileOpen(false)}
                    className="flex items-center gap-2 px-3 py-2.5 text-sm text-gray-700 hover:bg-blue-50 rounded-md">
                ...
                <>
                  <Link to="/login" className="flex-1">
                    <Button fullWidth variant="outline" radius="md" size="sm"
                      styles={{ root: { borderColor: "#1a6fa8", color: "#1a6fa8" } }}>
                      Login
                    </Button>
                  </Link>
